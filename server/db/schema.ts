...
export const subscription = mysqlTable("subscription", {
  subscription_id: bigint("subscription_id", { mode: "bigint", unsigned: true })
    .primaryKey()
    .autoincrement(),
  ...
  donor_external_id: varchar("donor_external_id", { length: 36 })
    .notNull()
    .references(() => donor.donor_external_id), //uuid,
...
export const transaction = mysqlTable("transaction", {
  ...
  subscription_external_id: varchar("subscription_external_id", {
    length: 36,
  }).references(() => subscription.subscription_external_id), //uuid
  stripe_subscription_id: varchar("stripe_subscription_id", { length: 100 })
    .notNull()
    .references(() => subscription.stripe_subscription_id),
  donation_project: varchar("donation_project", { length: 50 }).notNull(),
  amount: int("amount", { unsigned: true }).notNull(),
  currency: varchar("currency", { length: 3 }).notNull(),
  type: varchar("type", {
